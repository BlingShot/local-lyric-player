...
The analysis worker loads one local WASM binary instead of embedding it as a large base64 string. Runtime dependencies/codecs and licenses remain. `npm run desktop:budget` checks the staged app payload (<8 MiB), one WASM asset, absence of maps/logs/models/dev dependencies, and records installer sizes when present. This budget excludes Electron and the bundled native decoder; it is not an EXE size claim.
