...
interface Props {
  recordingWordIds?: readonly string[];
  syncTarget?: SyncTarget; clearSyncTarget?: () => void;
  advanced?: boolean;
...
export function StudioProjectRows({ draft, duration, issues, commit, select, listen, canListen, region, onError, selectedIds, setSelectedIds, advanced = true, syncTarget, clearSyncTarget, recordingWordIds = [] }: Props) {
  ...
  useEffect(() => { if (syncTarget && draft.settings.mode === 'word') setWordPanelOpen(true); }, [syncTarget?.revision, draft.settings.mode]);
  const selectedLine = draft.lines.find(line => line.id === draft.selectedId);
  const { gaps, focusId } = useProjectPlayback(draft, duration, canListen, region);
  const syncing = syncTarget?.lineId === draft.selectedId && focusId !== LYRIC_END;
  const follow = useLyricFollow(syncing ? draft.selectedId : focusId || draft.selectedId, `${draft.settings.mode}:${advanced}:${syncTarget?.revision || 0}`, true, draft.trackId);
  ...
  useLayoutEffect(() => {
    ...
    if (syncing) { follow.resume(); return; }
  ...
  }, [draft.selectedId, region, syncing, syncTarget?.revision]);
  ...
  const wordPanel = (line: VocalLine) => <details className='studio-detail studio-word-panel' open={wordPanelOpen}>
  ...
  const boundary = (id: string, label: string) => {
    ...
    return <div className='studio-row studio-sync-row studio-boundary' data-line-id={id} data-selected={draft.selectedId === id || undefined} data-sync-current={syncing && draft.selectedId === id || undefined} onFocusCapture={e => { if (!(e.target instanceof Element && e.target.closest("button"))) select(id); }} onClick={() => select(id)}>
  ...
  return <div className='studio-editor-panel'><div className='studio-editor-caption'><span>{t("Lyrics & timing")}</span>{canListen && <button aria-pressed={follow.following} onClick={() => { clearSyncTarget?.(); follow.following ? follow.browse() : follow.resume(); }}>{t(follow.following ? "Following playback" : "Follow playback")}</button>}</div><div className='studio-rows' ref={node => { region.current = node; follow.viewport.current = node; }} role='region' aria-label={t("Lyric editor")}
  ...
  </div>{advanced && selectedLine && draft.settings.mode === 'word' && wordPanel(selectedLine)}{!follow.following && <button className='studio-follow-button studio-sync-resume' onClick={follow.resume}>{t("Back to sync")}</button>}</div>;
}
