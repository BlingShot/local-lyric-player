...
export function StudioWorkspace({ trackId, changeTrack, seed }: { trackId: string; changeTrack: (id: string, seed?: StudioProject) => void; seed?: StudioProject }) {
  ...
  return <div className='offline-app studio-page' data-studio-format={surface} onDragOver={e => { e.preventDefault(); e.stopPropagation(); }} onDrop={e => { e.preventDefault(); e.stopPropagation(); const file = e.dataTransfer.files[0]; if (file) /\.(ttml|lrc|json)$/i.test(file.name) ? void importFile(file) : void chooseAudio(file); }}>
    ...
    <main className='studio-main' aria-label={t("Lyric Studio")}>
      ...
      {draft ? <>{surface === 'ttml' && <StudioInspector project={draft} commit={commit} selectedIds={selectedIds} />}<div className='studio-editor-layout'><StudioProjectRows advanced={surface === 'ttml'} draft={draft} duration={duration} issues={issues} commit={commit} select={select} listen={listen} canListen={canUseAudio} region={region} onError={setMessage} selectedIds={selectedIds} setSelectedIds={setSelectedIds} />{surface === 'ttml' && draft.settings.preview && <StudioLivePreview recordingWordId={recording.activeWordId} project={draft} durationMs={Math.round(duration * 1000)} enabled={canUseAudio} />}</div></> : <div className='studio-empty'>{t("Loading your draft…")}</div>}
    </main>
