...
export function MiniLyrics({ track, visible = true }: { track?: LocalTrack; visible?: boolean }) {
  ...
  return <div className='mini-lyrics' style={{ '--lyrics-background': color,
    ...
    {saved && track ? <LyricsView key={`${track.id}:${saved.savedAt}`} document={saved.document} trackId={track.id} offsetMs={timing.offsetMs} fontKey={`${fonts.lyrics.family}:${fontSize}`} visible={visible} />
