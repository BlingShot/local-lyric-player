...
interface Job { controller: AbortController; listeners: Set<(text: string) => void>; status: string; done: boolean; until: number; dismiss?: ReturnType<typeof setTimeout> }
const jobs = new Map<string, Job>();
function clearJobs() { for (const job of jobs.values()) { clearTimeout(job.dismiss); job.controller.abort(); } jobs.clear(); }
...
export function useResolvedLyrics(track?: LocalTrack) {
  ...
  useEffect(() => {
    ...
    if (!job || job.controller.signal.aborted || job.done && job.until < Date.now()) {
      if (job) clearTimeout(job.dismiss);
    ...
    return () => { current.listeners.delete(listener); if (!current.listeners.size && !current.done) { current.controller.abort(); clearTimeout(current.dismiss); if (jobs.get(key) === current) jobs.delete(key); } };
