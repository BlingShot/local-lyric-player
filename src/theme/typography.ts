...
const defaults: Typography = { app: { weight: 400, stretch: 100, spacing: 0, italic: false }, lyrics: { weight: 800, stretch: 100, spacing: 0, italic: false } };
...
export async function initializeTypography() {
  ...
  try { const saved = window.localMusicDesktop ? await window.localMusicDesktop.getConfig<Typography>('typography') : JSON.parse(localStorage.getItem('typography') || 'null'); if (initial !== revision) return; state = { values: { app: validAxes(saved?.app, defaults.app), lyrics: validAxes(saved?.lyrics, defaults.lyrics) }, error: '' }; publish(); }
